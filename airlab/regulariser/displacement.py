...
class JacobianRegulariser(_Regulariser):
    def __init__(self, pixel_spacing, size_average=True, reduce=True):
        super(JacobianRegulariser, self).__init__(pixel_spacing, size_average, reduce)
    ...
    def _joc_regulariser_3d(self,displacement):
        Neg_Jac = 0.5*(th.abs(self.Get_Ja(displacement)) - self.Get_Ja(displacement))
        #print("j: ",Neg_Jac.mean())
        return self._mask_3d(F.pad(Neg_Jac, (0, 1, 0, 1, 0, 1)))
...
class localRegulariser(_Regulariser):
    def __init__(self, pixel_spacing, size_average=True, reduce=True,vesselmask=None):
        super(localRegulariser, self).__init__(pixel_spacing, size_average, reduce)
        ...
        if self._dim == 2:
            ...
            self._regulariser = self._local_regulariser_3d  # 3d regularisation

    def _local_regulariser_2d(self, displacement):
        dx = th.abs(displacement[1:, 1:, :] - displacement[:-1, 1:, :])*self._pixel_spacing[0]
        dy = th.abs(displacement[1:, 1:, :] - displacement[1:, :-1, :])*self._pixel_spacing[1]

        return self._mask_2d(F.pad(dx + dy, (0, 1, 0, 1)))

    def _local_regulariser_3d(self, displacement):
        dx = th.abs(displacement[1:, 1:, 1:, :] - displacement[:-1, 1:, 1:, :])*self._pixel_spacing[0]
        dy = th.abs(displacement[1:, 1:, 1:, :] - displacement[1:, :-1, 1:, :])*self._pixel_spacing[1]
        dz = th.abs(displacement[1:, 1:, 1:, :] - displacement[1:, 1:, :-1, :])*self._pixel_spacing[2]
